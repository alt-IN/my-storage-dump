...
board_t * makeBoard(int w, int h, int numMines) {
  board_t  * initBoard = malloc(sizeof(*initBoard));
  initBoard->width = w;
  initBoard->height = h;
  initBoard->totalMines = numMines;
  initBoard->board=malloc(h*sizeof(*(initBoard->board)));
  for (int y = 0; y < h; y++) {
    initBoard->board[y]=malloc(w*sizeof(*(initBoard->board[y])));
  }
  for(int y = 0; y < h; y++) {
    for(int x = 0; x < w; x++) {
      initBoard->board[y][x]=UNKNOWN;
    }
  }
  for (int z = 0; z < numMines; z++) {
    addRandomMine (initBoard);
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int i = start_y; i <= y+1 && i < b->height; i++) {
    for (int j = start_x; j <= x+1 && j < b->width; j++) {
      if(IS_MINE(b->board[i][j]) && (j!=x || i!=y)) {
	count++;
      }
    }
  }
