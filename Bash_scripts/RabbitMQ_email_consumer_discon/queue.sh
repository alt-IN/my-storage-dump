...
BindEx=$(curl -s -u $USER:$PASS -H "content-type:application/json"     -XGET "http://$URL/api/exchanges/$VHOST/SxrcInR001/bindings/source" | python -m json.tool |  grep -o '"destination": ".*"' | sed -e 's/"destination": "\(.*\)"/\1/');
